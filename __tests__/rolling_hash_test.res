...
describe("Rolling Hash", () => {
  test("compare returns true when two hashs have seen the same sequence", () => {
    ...
    List.iter(n => IRH.append(h1, n), list{0, 1, 2, 3, 4, 5, 6, 7})
    List.iter(n => IRH.append(h2, n), list{0, 1, 2, 3, 4, 5, 6, 7})
  ...
  test("skips shifts the start of the sequence forward", () => {
    ...
    List.iter(n => IRH.append(h1, n), list{3, 4, 5, 6, 7})
    List.iter(n => IRH.append(h2, n), list{0, 1, 2, 3, 4, 5, 6, 7})
    List.iter(n => IRH.skip(h2, n), list{0, 1, 2})
  ...
  test("compare returns false when two hashs hold different sequences", () => {
    ...
    List.iter(n => IRH.append(h1, n), list{0, 1, 2, 3, 4, 5, 6, 7})
    List.iter(n => IRH.append(h2, n), list{0, 1, 2, 3, 4, 5, 6})
