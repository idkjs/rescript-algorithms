open Jest
open Expect

describe("Hashing With Open Addressing", () => {
  open HashtblWithOpenAddressing

  let rec generate_keys = x =>
    switch x {
    | 0 => list{}
    | n => list{n, ...generate_keys(n - 1)}
    }

  let num_keys = 400

  let keys = generate_keys(num_keys)

  let setup = () => {
    let hash = create(
      ~pre_hash=n => n,
      ~hash=(num_buckets, pre_hash, iter) => mod(pre_hash + iter, num_buckets),
      (),
    )
    List.iter(key => add(hash, key, string_of_int(key)), keys)
    hash
  }

  test("length", () => {
    let hash = setup()

    let length = length(hash)

    expect(length) |> toEqual(num_keys)
  })

  test("find", () => {
    let hash = setup()

    let result = List.map(key => find(hash, key), keys)
    let expected = List.map(key => string_of_int(key), keys)

    expect(result) |> toEqual(expected)
  })

  test("add overrides an existing binding", () => {
    let hash = setup()

    add(hash, 17, "Hi")
    let e17 = find(hash, 17)

    expect(e17) |> toEqual("Hi")
  })

  test("remove removes the key from the map", () => {
    let hash = setup()

    remove(hash, 17)

    expect(() => find(hash, 17)) |> toThrow
  })

  test("remove throws when the key doesn't exists", () => {
    let hash = setup()

    remove(hash, 17)

    expect(() => remove(hash, 17)) |> toThrow
  })
})
