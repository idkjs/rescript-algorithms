...
describe("Heap", () => {
  ...
  test("remove", () => {
    ...
    let match_ = (_, value, ~match_value) => value == match_value
    ...
    let removeSmallestN = n => {
      let rec rem = x =>
        ...
        | x =>
          ...
          remove(heap, match_(~match_value=value)) |> ignore
          rem(x - 1)
        }
      rem(n - 1)
    }
    ...
    expect((res1, res2, res3)) |> toEqual(("10", "11", "12"))
  })
  ...
  test("update", () => {
    ...
    let decrementSmallestN = n => {
      let rec rem = x =>
        ...
        | x =>
          let value = string_of_int(x)
          update(heap, match_(~match_value=value), x - 1, string_of_int(x - 1)) |> ignore
          rem(x - 1)
        }
      rem(n - 1)
    }
    ...
    expect((res1, res2, res3)) |> toEqual(("-1", "0", "1"))
  })
