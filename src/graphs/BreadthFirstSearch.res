...
let parseAdjList = adj_list => {
  ...
  let insert = ({id, neighbours}) => Hashtbl.add(adj_tbl, id, neighbours)

  List.iter(insert, adj_list)
  let validateNeighbours = node => List.iter(neighbour =>
...
let rec traverse = (~frontier, ~adj_tbl, ~level, ~parent, ~current_level) => {
  let updateFrontier = (_, node_id) => {
    let visit = visit(~adj_tbl, ~level, ~parent, ~current_level)
    visit(node_id)
  }

  let frontier_empty = List.length(frontier) == 0
  if !frontier_empty {
    let frontier = List.fold_left(updateFrontier, list{}, frontier)
    traverse(~frontier, ~adj_tbl, ~level, ~parent, ~current_level=current_level + 1)
  }
}
...
let search = (adj_list, root_id) => {
  ...
  traverse(~frontier=list{root_id}, ~adj_tbl, ~level, ~parent, ~current_level=0)

  let markNodeIfUnvisited = markNodeIfUnvisited(~level, ~parent)
