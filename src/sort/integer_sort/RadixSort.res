...
let list_to_array = list => {
  ...
  List.iteri((ind, e) => array[ind] = Some(e), list)
...
let list_to_bucket = (list, base, iteration) => {
  let buckets = Array.make(base, list{})

  let aux = num => {
    let bucket_index = mod(num / pow(base, iteration), base)
    let bucket = buckets[bucket_index]
    buckets[bucket_index] = list{num, ...bucket}
  }
...
let buckets_to_list = buckets =>
  Array.fold_left((bucket, list) => List.append(list, bucket), list{}, buckets)
